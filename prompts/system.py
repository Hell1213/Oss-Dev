...
def get_system_prompt(
    ...
    # OSS Dev Agent prompts (if enabled)
    if config.oss.enabled:
        parts.append(_get_oss_section())
...
def _get_oss_section() -> str:
    """Generate OSS Dev Agent section."""
    from prompts.oss import get_oss_identity_prompt, get_scope_discipline_prompt
    
    return f"""{get_oss_identity_prompt()}

{get_scope_discipline_prompt()}

## OSS Workflow Phases

When working on GitHub issues, follow the 7-phase workflow:

1. **Repository Understanding**: Analyze repo structure, create START_HERE.md
2. **Issue Intake**: Fetch and understand the GitHub issue
3. **Planning**: Locate code, form strategy (NO CODE YET)
4. **Implementation**: Make minimal, focused changes
5. **Verification**: Run tests, verify fix works
...
- `feat(scope): description` - for new features (rare in OSS fixes)
- `test(scope): description` - for test additions
- `docs(scope): description` - for documentation
- Always reference issue: `Fixes #123` or `Closes #123`
...
- Be respectful and professional in PR comments"""
